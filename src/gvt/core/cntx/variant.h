...
inline std::ostream &operator<<(std::ostream &os, const std::nullptr_t &ptr) {
...
template <typename... Ts> struct variant {
  ...
  template <typename U,
            typename = typename std::enable_if<check<U, Ts...>::value>::type>
  ...
  U to() {
    ...
    if (!is<U>())
      throw std::runtime_error("Error: incorrect data type request : " + std::to_string(tid) + " :: " + std::string(typeid(U).name()));
  ...
  U to() const {
    if (!is<U>())
      throw std::runtime_error("Error: incorrect data type request");
    return *reinterpret_cast<const U *>(_storage);
  }

  template <typename U> bool valid() { return check<U, Ts...>::value; }

  template <typename U> bool is(void) const {
    return tid == tindex<U, Ts...>::value;
  }
...
private:
  ...
  void safe_dealloc(TypeNumber<sizeof...(Us)> = TypeNumber<sizeof...(Us)>()) {
  ...
  template <typename U>
  void safe_copy(const variant &other, TypeNumber<0> = TypeNumber<0>()) {
    new (_storage) U(other.to<U>());
  }

  template <typename U, typename... Us>
  void safe_copy(const variant &other,
                 TypeNumber<sizeof...(Us)> = TypeNumber<sizeof...(Us)>()) {
    if (!other.is<U>())
      return safe_copy<Us...>(other, TypeNumber<sizeof...(Us) - 1>());
    new (_storage) U(other.to<U>());
  }

  template <typename U>
  void safe_move(variant &&other, TypeNumber<0> = TypeNumber<0>()) {
    new (_storage) U(std::move(other.to<U>()));
  }

  template <typename U, typename... Us>
  void safe_move(variant &&other,
                 TypeNumber<sizeof...(Us)> = TypeNumber<sizeof...(Us)>()) {
    if (!other.is<U>())
      return safe_move<Us...>(std::move(other),
                              TypeNumber<sizeof...(Us) - 1>());
    new (_storage) U(std::move(other.to<U>()));
  }
  ...
  template <typename U>
  std::ostream &_print(std::ostream &os,
                       TypeNumber<0> = TypeNumber<0>()) const {
    return os << to<U>();
  }
  ...
  _print(std::ostream &os,
         TypeNumber<sizeof...(Us)> = TypeNumber<sizeof...(Us)>()) const {

    if (not is<U>())
      return _print<Us...>(os, TypeNumber<sizeof...(Us) - 1>());
  ...
  template <typename U>
  void _pack(cntx::mpi::encode &enc, TypeNumber<0> = TypeNumber<0>()) {
    enc.pack<U>(to<U>());
  }

  template <typename U, typename... Us>
  void _pack(cntx::mpi::encode &enc,
             TypeNumber<sizeof...(Us)> = TypeNumber<sizeof...(Us)>()) {
    if (is<U>()) {
      enc.pack<U>(to<U>());
    } else {
  ...
  template <typename U>
  void _unpack(cntx::mpi::decode &dec, TypeNumber<0> = TypeNumber<0>()) {
  ...
  template <typename U, typename... Us>
  void _unpack(cntx::mpi::decode &dec,
               TypeNumber<sizeof...(Us)> = TypeNumber<sizeof...(Us)>()) {
    if (is<U>()) {
      (*this) = dec.unpack<U>();
    } else {
...
public:
  ...
  typename std::aligned_storage<sizeof(max_type), alignof(max_type)>::type
      _storage[max_size];
