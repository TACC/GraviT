...
struct OptixParallelTrace
{
    ...
    /**
     * Construct a OptixParallelTrace struct with information needed for the thread
     * to do its tracing
     */
    OptixParallelTrace(
            gvt::render::adapter::optix::data::OptixMeshAdapter* adapter,
    ...
    void operator()()
    {
        ...
        {
#ifdef GVT_USE_DEBUG
            // boost::timer::auto_cpu_timer t_outer_loop("OptixMeshAdapter: workSize rays traced: %w\n");
#endif
            ...
            GVT_DEBUG(DBG_ALWAYS, "OptixMeshAdapter: working on rays [" << workStart << ", " << workEnd << "]");
...
void OptixMeshAdapter::trace(gvt::render::actor::RayVector& rayList, gvt::render::actor::RayVector& moved_rays, gvt::core::DBNodeH instNode)
{
#ifdef GVT_USE_DEBUG
    ...
    GVT_DEBUG(DBG_ALWAYS, "OptixMeshAdapter: trace: instNode: " << gvt::core::uuid_toString(instNode.UUID()) << ", rays: " << rayList.size() << ", workSize: " << workSize << ", threads: " << gvt::core::schedule::asyncExec::instance()->numThreads);
    ...
    GVT_DEBUG(DBG_ALWAYS, "OptixMeshAdapter: converted " << lightNodes.size() << " light nodes into structs: size: " << lights.size());
    // end `convertLights`
    //
