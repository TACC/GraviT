                ...
                struct AdaptiveSendSchedule : public HybridScheduleBase 
                {
                    ...
                    virtual void operator()() 
                    {
                        ...
                        for (std::map<int, int>::iterator it = data2size.begin(); it != data2size.end(); ++it) 
                        {
                            ...
                            else 
                            {
                                ...
                                if (pdit != procs_count.end()) 
                                {
                                    ...
                                    {
                                        GVT_DEBUG(DBG_LOW,"            adding " << cur_data2procs[pdit->first][i] << " to zero-priority pool at " << i);
                                        zero_priority[i].push_back(cur_data2procs[pdit->first][i]); // push proc id onto stack, let map sort into use-first order
                                    }
                                    avail += pdit->second;
                                    GVT_DEBUG(DBG_LOW,"        adding " << pdit->second << " zero-priority procs to available pool, is now " << avail);
                                }
                            }
                        }
                        ...
                        if (!priority.empty()) 
                        {
                            for (std::map<int, std::vector<int> >::iterator it = cur_data2procs.begin(); it != cur_data2procs.end(); ++it) 
                            {
                                if (data2size.find(it->first) == data2size.end()) 
                                {
                                    ...
                                    for (int i = 0; i < pc; ++i) 
                                    {
                                        GVT_DEBUG(DBG_LOW,"            adding " << it->second[i] << " to zero-priority pool at " << ((pc << 6) + i));
                        ...
                        for (std::map<int, std::vector<int> >::iterator it = send_data.begin(); it != send_data.end(); ++it) 
                        {
                            ...
                            GVT_DEBUG(DBG_LOW,"    data " << it->first << " needed on " << to_recv << " procs, already on " << to_send << " procs");
                            for (int r = 0, s = 0; r < to_recv; ++r, ++s) 
                            {
                                s = r % to_send; // XXX TODO pnav: 's' is likely an error here, but confirm and fix
                                GVT_DEBUG(DBG_LOW,"    for data " << it->first << ": " << it->second[r] << " <- " << cur_data2procs[it->first][s]);
                        ...
                        GVT_DEBUG_CODE(DBG_LOW,
                            std::cerr << "data_send buffer is" << std::endl;
                            for (int i = 0; i < size; ++i)
                                if (data_send_buf[i] < 0)
                                    std::cerr << "    " << i << " keeps" << std::endl;
                                else
                                    std::cerr << "    " << i << " <-- " << data_send_buf[i] << std::endl;
                                );
                    }
                };
            }
