...
#ifndef GVT_LOADANYONCESCHEDULE_H
#define	GVT_LOADANYONCESCHEDULE_H

#include "HybridBaseSchedule.h"

struct LoadAnyOnceSchedule : public HybridBaseSchedule {

    LoadAnyOnceSchedule(int * newMap, int &size, int *map_size_buf, int **map_recv_bufs, int *data_send_buf) : HybridBaseSchedule(newMap, size, map_size_buf, map_recv_bufs, data_send_buf) {
    ...
    virtual void operator()() {
        DEBUG(cerr << "in LoadAnyOnce schedule" << endl);
...
#endif	/* GVT_LOADANYONCESCHEDULE_H */
