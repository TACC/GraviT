...
# parts of the GraviT API. It is intended to be run on exactly 2 mpi
# ranks and will halt if you try to run it on any other number of ranks.
# The code is hardwired to split the volume into a top and bottom part
# and to use the domain schedule and ospray adapter. Messing with those
# settings will have unpredictable results.
#
# run with this command "ibrun -np 2 python gvtVol_parallel.py". Or use
# your favorite mpi launcher.
#
import gvt
...
if rank == 0:
    gvt.addVolumeSamples(volnodename, bottomvol, counts, originbottom, deltas,
                         samplingrate)
else:
    gvt.addVolumeSamples(volnodename, topvol, counts, origintop, deltas,
                         samplingrate)
...
rendername = "PythonVolRenderer_parallel"
